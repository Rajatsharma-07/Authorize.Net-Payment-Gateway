...
const recurring = (cardNumber, expiryDate, cvv, amount, timePeriod, planId, user, currPlan, timeLimit, occurrences, limit, remainingBalance) => { new Promise((resolve, reject) => {
    ...
    function getSubsResponse(response, subsId){
        ...
        cardFields.cardNumber = card_number;
        cardFields.expiryDate = expiry_date;
        ...
        var remainingamount = parseInt(remainingBalance);
        var patientLimit = parseInt(limit);
        
        
        var subsfield= {};
        subsfield.user = user;
        subsfield.planId = planId;
        ...
        var cartField = {};
        cartField.user = user;
        cartField.planId = planId;
        cartField.totalAmount = response.subscription.amount;

        Subs.findOneAndUpdate({subscriptionId : currPlan}, {$set : {status : "expired"}}, (err, result) => {
            if(err){
                console.error(err);
            }else{
                console.log(result);
    ...
    function createSubscription(callback){
        ...
        intervalType.setLength(timeLimits);
        ...
        paymentScheduleType.setTotalOccurrences(occurrence);
